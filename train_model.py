...
# The reason we need to create a separate scaler for the input values and the output values is that well want to be
# able to use them separately when we make real predictions with the model. Scale both the training inputs and outputs
X[X.columns] = X_scaler.fit_transform(X[X.columns])
y[y.columns] = y_scaler.fit_transform(y[y.columns])

# pandas dataframes into numpy arrays. becuase Tf does not support pandas datfrmaes
...
# Earlier, we talked about hidden layers with four nodes. But in real life, we'll often use hundreds of nodes in a
# single layer.
